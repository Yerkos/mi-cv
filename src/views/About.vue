<script setup lang="ts"></script>

<template>
  <div class="container">
    <section class="section-container">
      <span class="about-subtitle">ABOUT ME</span>
      <div class="section-container-header">
        <h1>Aitor Ramirez</h1>
        <span class="about-subtitle-main"
          >Frontend Developer · Las Palmas de Gran Canaria</span
        >

        <nav class="section-container-nav">
          <a
            class="section-container-nav-icons"
            href="mailto:aitor_rg1993@hotmail.com"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
              stroke-linecap="round"
              stroke-linejoin="round"
            >
              <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
              <rect x="3" y="5" width="18" height="14" rx="2"></rect>
              <polyline points="3 7 12 13 21 7"></polyline>
            </svg>
            aitor_rg1993@hotmail.com
          </a>
          <a
            class="section-container-nav-icons"
            href="https://aitor.dev"
            target="_blank"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
              stroke-linecap="round"
              stroke-linejoin="round"
            >
              <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
              <circle cx="12" cy="12" r="9"></circle>
              <path
                d="M3.6 9h16.8M3.6 15h16.8M11.5 3a17 17 0 0 0 0 18M12.5 3a17 17 0 0 1 0 18"
              ></path>
            </svg>
            aitor.dev
          </a>

          <a
            class="section-container-nav-icons"
            href="ITResume.pdf"
            download="Aitor-Ramirez-Resume.pdf"
          >

          
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="1"
              stroke-linecap="round"
              stroke-linejoin="round"
              class="icon icon-tabler icons-tabler-outline icon-tabler-file-cv"
            >
              <path stroke="none" d="M0 0h24v24H0z" fill="none" />
              <path d="M14 3v4a1 1 0 0 0 1 1h4" />
              <path
                d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2"
              />
              <path d="M11 12.5a1.5 1.5 0 0 0 -3 0v3a1.5 1.5 0 0 0 3 0" />
              <path d="M13 11l1.5 6l1.5 -6" />
            </svg>
            Resume
          </a>
        </nav>
      </div>

      <p class="section-container-text">
        Passionate about crafting seamless user experiences and writing clean,
        efficient code. I specialize in building intuitive and visually
        appealing interfaces that solve real-world problems. By blending
        technical expertise with creative design thinking, I deliver products
        that make an impact — always striving for improvement and innovation,
        one line of code at a time.
      </p>
    </section>

    <!-- section exp -->
    <section class="section-container">
      <div class="section-exp-title">
        <h3>Frontend Developer & Support</h3>
        <span>2025 - 2026</span>
      </div>

      <a href="https://www.planimatik.com/" target="_blank">
        Planimatik - Remote</a
      >
      <div class="section-exp-info">
        <ul>
          <li>
            Contributed to the Cargobot project by designing and implementing
            new frontend sections and supporting clients problems with the app.
          </li>
        </ul>
      </div>

      <div class="section-exp-title">
        <h3>Fronted Developer</h3>
        <span>2022 - 2025</span>
      </div>

      <a href="https://www.singularfactory.com/" target="_blank">
        The singular factory - Remote</a
      >

      <div class="section-exp-info">
        <ul>
          <li>
            Contributed to the development of a frontend application Planimatik
            from scratch, participating in architecture decisions and
            implementation.
          </li>
          <li>
            Contributed to the Cargobot project by designing and implementing
            new frontend sections and features based on client requirements.
          </li>
        </ul>
      </div>
    </section>

    <section class="section-container">
      <h3>PROJECTS</h3>

      <div class="projects-grid">
        <div class="section-project-card-container">
          <div class="section-project-card-container-title">
            <h3>Web Page</h3>
            <a
              href="https://github.com/Yerkos/mi-cv"
              target="_blank"
              rel="noopener noreferrer"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="2.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              >
                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                <path
                  d="M12 6h-6a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-6m-7 1l9-9m-5 0h5v5"
                ></path>
              </svg>
              https://github.com/Yerkos/mi-cv
            </a>
          </div>

          <p>Personal Curriculum Page</p>
        </div>

        <div class="section-project-card-container">
          <div class="section-project-card-container-title">
            <h3>AppleTrack</h3>
            <a
              href="https://github.com/Yerkos/appletack"
              target="_blank"
              rel="noopener noreferrer"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="2.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              >
                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                <path
                  d="M12 6h-6a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-6m-7 1l9-9m-5 0h5v5"
                ></path>
              </svg>
              github.com/aitor/snapstack
            </a>
          </div>

          <p>AppleWatch APP to track heartRate zones while running</p>

          <div class="section-project-card-container-tec">
            <div class="section-project-card-container-tec-cards">
              <span>CSS / Tailwind</span>
            </div>
            <div class="section-project-card-container-tec-cards">
              <span>vue</span>
            </div>
            <div class="section-project-card-container-tec-cards">
              <span>typescript</span>
            </div>
          </div>
        </div>
      </div>
    </section>

    <!-- skills -->

    <section class="section-container"></section>
  </div>
</template>
